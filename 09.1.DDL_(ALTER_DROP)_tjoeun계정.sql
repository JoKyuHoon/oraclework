...
-- 오류 :컬럼값에 문자가 포함되어 있어서 실행되지 않음. 컬럼의 타입을 변경하려면 컬럼값들을 모두 지워야 변경 가능하다.
ALTER TABLE DEPT_COPY MODIFY DEPT_ID NUMBER; 
-- 오류 : 컬럼의 타입을 변경하려면 컬럼값들을 모두 지워야 변경 가능하다.
...
-- LOCATION_ID VARCH2(2)
-- LNAME 컬럼의 기본값을 '미국'으로 변경하시오
...
-- 오류 : 한 테이블에 최소 1개의 컬럼은 가지고 있어야한다.
ALTER TABLE DEPT_COPY2 
...
*/
--DEPT_COPY 테이블에서 DEPT_TITLE 을 DEPT_NAME으로 변경
ALTER TABLE DEPT_COPY 
    RENAME COLUMN DEPT_TITLE TO DEPT_NAME;

-- DEPT_COPY 테이블명을 DEPT_TEST로 변경
