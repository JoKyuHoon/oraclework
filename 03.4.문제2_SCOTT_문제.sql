...
SELECT ENAME, EXTRACT(YEAR FROM HIREDATE) 년도, EXTRACT(MONTH FROM HIREDATE) 월, EXTRACT(DAY FROM HIREDATE) 일
FROM EMP;
...
SELECT EMPNO, ENAME, HIREDATE,  EXTRACT(YEAR FROM HIREDATE)+40 || '/' || EXTRACT(MONTH FROM HIREDATE) || '/' || EXTRACT(DAY FROM HIREDATE)
FROM EMP;

SELECT EMPNO, ENAME, HIREDATE, TO_CHAR(ADD_MONTHS(HIREDATE, 40 * 12), 'YYYY/MM/DD') AS PROMOTION_DATE
FROM EMP;
