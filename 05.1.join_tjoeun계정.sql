/*
    ...
    - JOIN은 크게 "오라클전용구문"과 "ANSI 구문"(ANSI == 미국국립표준협회에서 만듦)
...
/*
    1. 등가조인(EQUAL JOIN) / 내부조인|(INNER JOIN)
       연결시키는 컬럼의 값이 "일치하는 행들만" 조인되어 조회한다(일치하지 않으면 조회X)     
*/
-- >> 오라클 전용구문
-- FROM절에 조회하고자하는 테이블들을 나열(, 를 기준으로)
-- WHERE 절에 매칭시킬 컬럼(연결고리)에 대한 조건 제시

-- 1) 연결할 두 컬럼명이 서로 다른 경우(EMPLOYEE: DEPT_CODE, DEPARTMENT: DEPT_ID)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT;
-- 인사관리부 23번, 회계관리부 23번, ... << 이런식으로 조회됨
...
/*
    <포괄조인 / 외부조인(OUTER JOIN>
      두 테이블 간의 JOIN시 일치하지 않는 행도 포함시켜 조회
      단, 반드시 LEFT / RIGHT를 지정해야 한다(기준이 되는 테이블을 지정)
*/

-- 모든 사원의 사원명, 부서명, 급여, 연봉(부서를 배치받지 못한 사원도 포함) 조회
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12 AS 연봉
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
-- 위 구문은 부서배치가 안된 사원 2명은 제외되었음
...
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12 AS 연봉
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+); -- 기준이 되는 테이블 반대편 테이블 컬럼 뒤에 (+)를 붙여준다
...
*/
-- 전체 사원의 사번, 사원명, 부서코드, (EMPLOYEE E 테이블 에서 조회)
-- 사수사번, 사수명, 사수부서코드 조회 (EMPLOYEE M 테이블 에서 조회)
-->> 오라클 전용 구문
-- 1) 사수가 있는 사원만 조회
