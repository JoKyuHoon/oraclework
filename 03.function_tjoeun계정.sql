...
*/
 ...
 /*
    * INSTR : 문자열로 부터 특정 문자의 시작위치(INDEX)를 찾아서 반환(반환형 : NUMBER)
    - ORACLE은 INDEX번호가 1번부터 시작, 찾는 문자가 없을 때 0반환
...
SELECT EMAIL, INSTR(EMAIL,'_') "_의 위치", INSTR(EMAIL,'@') "@의 위치" FROM EMPLOYEE;
 ...
 */
 ...
 -- EMPLOYEE 테이블에서 주민번호 중 성별만 추출하여 사원명, 주민번호, 성별을 조회
 ...
 -- EMPLOYEE 테이블 EMAIL에서 아이디만 추출하여 사원명, 이메일, 아이디를 조회
...
-- EMPLOYEE테이블에서 주민번호를 123456-1****** 형식으로 사번, 사원명, 주민번호 조회
...
FROM EMPLOYEE;
 ...
 /* 
    ...
    * TRIM : 문자열의 앞/뒤 양쪽에 있는 특정 문자를 제거한 나머지 반환
    ...
    TRIM([LEADING|TRAILING|BOTH] 제거하고자하는 문자 FROM'문자열') --> 제거하고자하는 문자는 1개만 가능
 */
 ...
 SELECT LTRIM('JAVASCRIPTJAVA', 'JAVA') FROM DUAL;
 SELECT LTRIM('ABCABCABCFQWER', 'ABC') FROM DUAL;
 SELECT LTRIM('451347DK45675', '0123456789') FROM DUAL;
 
 SELECT RTRIM('JAVASCRIPTJAVA', 'JAVA') FROM DUAL;
 SELECT RTRIM('ABCABCABCFQWER', 'ABC') FROM DUAL;
 SELECT RTRIM('451347DK45675', '0123456789') FROM DUAL;
 
 SELECT TRIM('     TJOEUN     ') || '더조은' FROM DUAL;
 SELECT TRIM('A' FROM 'AAAT1AAA') FROM DUAL;
 
 SELECT TRIM(LEADING 'A' FROM 'AAAT1AAA') FROM DUAL;
 SELECT TRIM(TRAILING 'A' FROM 'AAAT1AAA') FROM DUAL;
 SELECT TRIM(BOTH 'A' FROM 'AAAT1AAA') FROM DUAL;
 SELECT TRIM('A' FROM 'AAAT1AAA') FROM DUAL;  -- BOTH 기본값 생략가능
 
  -----------------------------------------------------------------------------------------------------------------------------
 
 /*
    * LOWER / UPPER/ INITCAP : 문자열을 모두 대문자 혹은 소문자, 첫글자만 대문자로 변환(반환형: CHARACTER)
 ...
 /*
    * CONCAT : 문자열 2개를 전달받아 하나로 합친 문자반환
 ...
 */
 ...
 -- SELECT CONCAT('Oracle', '오라클', '아') from dual;   << 문자열 2개만 가능
 SELECT 'Oracle' || '오라클' || '우아앙' || '깡' FROM DUAL; -- << ||은 가능
...
FROM EMPLOYEE;
-- ↕ 둘이 결과값이 같음
SELECT EMP_NAME, SALARY*NVL(1+BONUS, 1) * 12  -- 곱하기니까 0이 아닌 1로 해야됨.
FROM EMPLOYEE;
 ...
 */
 ...
 -- 사원명, 급여, 등급(급여에 따른 등급(5백만원 이상이면 '고급', 5백만원~3백만원 '중급', 나머지 '초급'))
...
/*
    * SUM(숫자타입의 컬럼) : 해당 컬럼 값들의 총합을 반환해주는 함수
*/
-- 전 사원의 총 급여의 합
...
-- 남자 사원의 총 급여의 합
SELECT SUM(SALARY) AS "남자 사원의 총 급여의 합"
FROM EMPLOYEE
...
-- 부서코드가 D5인 사원의 보너스를 포함한 연봉의 합
SELECT SUM(SALARY*NVL(1+BONUS, 1)) * 12 "보너스포함 연봉의 합"
FROM EMPLOYEE
WHERE DEPT_CODE ='D5';

-- 전 사원의 총 급여의 합 형식 \000,000,000
SELECT TO_CHAR(SUM(SALARY), 'L999,999,999') AS "총 급여의 합"
FROM EMPLOYEE;
