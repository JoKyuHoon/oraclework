/*
    * SEQUENCE
       : 자동으로 번호를 발생시켜주는 객체(정수값을 순차적으로 일정값씩 증가시키면서 생성)
       
       EX) 회원번호, 사원번호, 게시글번호, 주문번호, ... (PRIMARY KEY)
*/
/*
    * SEQUENCE 객체 생성
    ...
    [STRAT WITH 시작숫자]             -> 처음 발생시킬 시작값 지정 넣지 않으면 기본값은 1, 10을 넣으면 10, 11, 12, ...
    ...
    [NOCACHE | CACHE 바이트크기] -> 캐시메모리 할당(기본값 CACHE 20)
                                                * 캐시메모리 : 시퀀스 값을 미리 저장해 두는 곳
                                                  매번 호출할 때 마다 새롭게 번호를 생성하는게 아니라
                                                  캐시메모리에 미리 만들어 둔 값을 가져다 씀
                                                  -> 접속 해제시 캐시메모리에 미리 만둘어 둔 번호는 다 날아감
...
/*
    * SEQUENCE 사용
      ...
      시퀀스명.NEXTVAL : 시퀀스값에 일정하게 증가시켜서 발생된 값
                                 ...
                                 시퀀스.CURRVAL + INCREMENT BY한 값
*/
SELECT SEQ_EMPNO.CURRVAL FROM DUAL;
-- NEXTVAL을 한번도 수행하지 않은 상태. 성공한적 없음 CURRVAL을 사용할 수 없음
