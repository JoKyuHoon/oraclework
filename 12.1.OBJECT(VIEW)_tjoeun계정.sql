/*
    * VIEW : SELECT문을 저장해줄 수 있는 객체
        -> 임시테이블(논리적인 테이블) : 실제로 존재하지 않는 테이블
    - 자주 사용하는 긴 SELECT문에 사용
...
-- 관리자계정으로 바꾼 후 tjoeun view 생성 권한
...
SELECT * FROM (SELECT EMP_ID, EMP_NAME, DEPT_TITLE, SALARY, NATIONAL_NAME
                        ...
                        JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
                        JOIN NATIONAL USING(NATIONAL_CODE));
-- VIEW는 논리적인 가상 테이블(실질적으로 데이터를 저장하고 있지 않음)
...
/*
    * VIEW 컬럼에 별칭 부여
      서브쿼리의 SELECT절에 함수식이나 산술연산식이 기술되어 있는 경우는 반드시 별칭을 부여해야한다
      
      - CREATE OR REPLACE VIEW 뷰명 : 뷰명이 기존에 만들어놓은 뷰명이면 덮어쓰기
*/
-- 전 사원의 사번, 사원명, 직급명, 성별(남/여), 근무년수 조회 SELECT문을 VIEW로 정의하시오
...
-- 뷰를 통해 삽입
INSERT INTO VW_JOB VALUES('J8', '인턴'); -- JOB 테이블에도 삽입
...
/*
    * DML 명령어로 조작이 불가능한 경우
      1) 뷰에 정의되지 않은 컬럼을 조작하고자 하는 경우
      2) 뷰에 정의되어 있는 컬럼 중 테이블에 NOT NULL제약조건이 지정되어 있는 경우
...
-- 실제 테이블에 (NULL, 인턴)이 추가됨 JOB_CODE는 NOT NULL이기 때문에 오류
...
-- INSERT
INSERT INTO VW_GROUP VALUES('D3', 80000000, 40000000); -- 오류

-- UPDATE
UPDATE VW_GROUP
SET 합계 = 80000000
WHERE DEPT_CODE = 'D1'; -- 오류
