 /*
    * DML(DATA MAINPLATION LANGUAGE) : 데이터 조작언어
 ...
 /*
    1. INSERT
      ...
      - INSERT INTO 테이블명 VALUES(값1, 값2, 값3, ...)
        테이블에 모든 컬럼에 대한 값을 넣고자 할 때(컬럼의 순서를 지켜야한다)
 ...
 */
 ...
 INSERT INTO EMPLOYEE VALUES(300, '더조은', '9909024-1122334', 'www@naver.com','01012341234'
                                             , 'D2', 'J1', 3500000, 0.2, SYSDATE, NULL, 'N');
 
 /*
    2) INSERT INTO 테이블명 (컬럼명1, 컬럼명2, 컬럼명3, ...) VALUES (값1, 값2, 값3, ...)
    : 테이블에 내가 선택한 컬럼에 대한 값만 INSERT할 때
    -> 그래도 한 행 단위로 추가됨.
    -> 선택이 안된 컬럼은 NULL이 들어감. DEFAULT가 지정되어 있으면 NULL이 아닌 DEFAULT 값이 들어감
...
/*
    3) INSERT INTO 테이블명 (서브쿼리)
      : 서브쿼리로 조회된 결과값을 모두 INSERT함(여러행INSERT가능)
*/
...
-- 급여가 3백만원보다 많이 받는 사람을 넣을 테이블, 적게받는 사람 테이블
...
WHEN SALARY > 3000000 THEN
    INTO EMP_HIGH VALUES (EMP_ID, EMP_NAME, SALARY, BONUS)
WHEN SALARY <= 3000000 THEN
    INTO EMP_LOW VALUES (EMP_ID, EMP_NAME, SALARY, BONUS)
...
-- 왕정보 사원의 급여와 보너스를 지정보 사원과 동일하게 인상
UPDATE EMP_SALARY
...
-- 조정연, 유하보, 강정보 직원들의 급여와 보너스를 문정보 사원과 동일하게
UPDATE EMP_SALARY
SET(SALARY, BONUS) = (SELECT SALARY, BONUS
                                FROM EMP_SALARY
                                WHERE EMP_NAME = '문정보')
WHERE EMP_NAME IN ('강정보', '조정연', '유하보');

-- EMP_COPY 테이블의 ASIA 지역에서 근무하는 사원들의 보너스를 0.3으로 변경

-- ASIA 에서 근무하는 사원들의 EMP_ID
SELECT EMP_ID
FROM EMPLOYEE_COPY
