...
-- DEPARTMENT테이블의 부서코드, 부서명 조회
SELECT DEPT_ID, DEPT_TITLE
FROM department;

-- EMPLOYEE테이블에서 사원명, 이메일, 전화번호, 입사일, 급여 조회
...
/*
    <컬럼값을 통한 산술연산>
    SELECT절 컬럼명 작성부분에 산술연산을 기술할 수 있음(산술연산된 결과 조회)
*/
...
FROM employee;
--> 산술연산 과정 중 NULL값이 존재할 경우 산술연산한 결과값도 무조건 NULL값으로 나온다

-- EMPLOYEE 테이블에서 사원명, 입사일, 근무일수 조회
-- DATE형식도 연산가능함 : 결과값은 일수로 나옴
-- 오늘 날짜 : SYSDATE
SELECT EMP_NAME, HIRE_DATE, SYSDATE-HIRE_DATE
FROM employee;
--> 근무일수에 소수점이하는 시분초 단우로 계산하기 때문

--------------------------------------------------------------------------------------------------------
/*
    <컬럼명에 별칭 지정하기>
    산술연산시 컬럼명이 산술에 들어간 수식 그대로 컬럼명이 지정됨. 이때 별칭을 부여할 수 있다
...
/*
    ...
    SELECT절에 리터럴을 넣으면 마치 TABLE상에 존재하는 데이터처럼 조회 가능
...
/*
    <연결 연산자 : >
    여러 컬럼값들을 마치 하나의 컬럼인것처럼 연결하거나, 컬럼값의 리터럴을 연결할 수 있음
*/
...
SELECT EMP_NAME || '의 월급은 ' || SALARY ||'원 입니다' "월급 정보"
FROM EMPLOYEE;
...
-- 오류 : DISTINCT는 SELECT절에서 한 번만 기술 가능
-- SELECT DISTINCT JOB_CODE, DISTINCT DEPT_CODE
-- FROM EMPLOYEE;

-- (J1-D1, J1-D2) << 이와같이 조합으로 겹치지 않는것 조회
...
/*
    ...
    'oracle%'
     ...
     oracleasd
        ...
        비교대상컬럼 LIKE '%문자%' => 비교대상 컬럼값이 '문자' 가 포함되어있는 데이터 조회
        
      -> '_' :  1글자
      ex) 비교대상컬럼 LIKE '_문자' => 비교대상 컬럼값이 '문자' 앞에 무조건 한글자가 있는 데이터 조회
           비교대상컬럼 LIKE '_ _문자' => 비교대상 컬럼값이 '문자' 앞에 무조건 두글자가 있는 데이터 조회
...
-- EMPLOYEE 테이블에서 사원들 이름에 '하'가 포함된 사원들의 사원명, 주민번호, 전화번호 조회
SELECT EMP_NAME, EMP_NO, PHONE
FROM EMPLOYEE
WHERE EMP_NAME LIKE '%하%';

-- EMPLOYEE 테이블에서 사원들 이름에 '하'가 포함된 사원들의 사원명, 전화번호 조회
SELECT EMP_NAME, PHONE
FROM EMPLOYEE
WHERE EMP_NAME LIKE '_하_';

-- EMPLOYEE 테이블에서 전화번호의 세번째 자리가 '1' 인 사원의 사번, 사원명, 전화번호 조회
...
FROM EMPLOYEE
WHERE PHONE LIKE '____%'; -- 언더바 4개로 인식해서 글자가 4개 이상인 데이터를 가져옴. -> 원하는 결과가 아님.
/*
    언더바( _ ) 가 와일드카드 인지 데이터값인지 구분지어야 함
    -> 데이터값으로 취급하고자하는 값 앞에 나만의 와일드카드(아무거나 가능)를 제시하고 ESCAPE에 등록해주면 된다.
        * 특수기호 중 '&' 를 사용하면 오라클에서는 사용자로부터 입력받는 키워드이므로 안 쓰는 것이 좋다.
  */
SELECT EMP_NAME, EMAIL
FROM EMPLOYEE
...
--EMPLOYEE테이블에서 보너스를 받지 않는 사원들의 사번, 이름, 사원명, 급여, 보너스 조회
SELECT EMP_ID, EMP_NAME, SALARY, BONUS
FROM EMPLOYEE
WHERE BONUS IS NULL;

--EMPLOYEE테이블에서 보너스를 받는 사원들의 사번, 이름, 사원명, 급여, 보너스 조회
SELECT EMP_ID, EMP_NAME, SALARY, BONUS
FROM EMPLOYEE
...
--EMPLOYEE테이블에서 사수가 없는 사원들의 사원명, 부서코드, 사수번호를 조회
SELECT EMP_NAME, DEPT_CODE, MANAGER_ID
FROM EMPLOYEE
WHERE MANAGER_ID IS NULL;

--EMPLOYEE테이블에서 부서배치를 받지 못했지만 보너스를 받는 사원들의 사번, 사원명, 부서코드를 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE
FROM EMPLOYEE
...
/*
    ...
    NOT IN : 컬럼값이 내가 제시한 목록중에 일치하는 값을 제외한 데이터만 조회
...
--EMPLOYEE 테이블에서 부서코드가 D6이거나 D8이거나 D9인 사원을 제외한사원명, 부서코드, 급여조회
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
--EMPLOYEE 테이블에서 직급코드가 'J7'이거나 'J2'인 직원들 중 급여가 200만원 이상인 사원들의 모든 컬럼 조회
SELECT *
FROM EMPLOYEE
...
-- EMPLOYEE 테이블의 모든 컬럼 조회. 정렬기준 BONUS의 내림차순 정렬, 보너스가 같으면 급여의 오름차순 정렬로 조회
SELECT *
FROM employee
...
SELECT EMP_NAME, SALARY, ((SALARY*BONUS + SALARY)*12 - (SALARY*0.03)) 실수령액, HIRE_DATE
FROM EMPLOYEE
...
SELECT *
FROM EMPLOYEE
WHERE HIRE_DATE BETWEEN '90/01/10' AND '01/01/01';
